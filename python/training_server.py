...
from side_ear_utils import (
    dataset_yaml_config as side_ear_dataset_yaml_config,
    ensure_dataset_layout as side_ear_ensure_layout,
    export_side_ear_web_library,
    list_images as list_side_ear_images,
    read_side_ear_labels,
    split_train_to_val as side_ear_split_val,
    write_side_ear_labels,
)
...
DATA_YAML_SIDE = ROOT / "data.side-ear.yaml"
ONNX_PATH_SIDE = ROOT / "public" / "models" / "side-ear.onnx"
SIDE_EAR_WEB_DIR = WEB_DIR / "side-ear"
...
class SaveSideEarAnnotation(BaseModel):
    root: str
    split: str = "train"
    filename: str
    image_width: int
    image_height: int
    points: list[EarPoint]
...
def save_side_ear_data_yaml_for_root(root: Path) -> None:
    cfg = side_ear_dataset_yaml_config(root)
    with DATA_YAML_SIDE.open("w", encoding="utf-8") as f:
        yaml.dump(cfg, f, default_flow_style=False, sort_keys=False)
...
# --- Side-ear training (4 keypoints, separate dataset + export) ---


@app.post("/api/side-ear/annotate/init")
def api_side_ear_init(body: DatasetRoot):
    root = resolve_root(body.root)
    side_ear_ensure_layout(root)
    save_side_ear_data_yaml_for_root(root)
    items = list_side_ear_images(root)
    return {
        "ok": True,
        "root": str(root),
        "total": len(items),
...
@app.post("/api/side-ear/annotate/upload")
async def api_side_ear_upload(
    root: str = Query(...),
    files: list[UploadFile] = File(...),
):
    dataset = resolve_root(root)
    side_ear_ensure_layout(dataset)
    train_dir = dataset / "images" / "train"
    saved: list[str] = []
    for uf in files:
        if not uf.filename:
        ...
        saved.append(dest.name)
    items = list_side_ear_images(dataset)
    return {
        "ok": True,
        "saved": saved,
        "total": len(items),
...
@app.get("/api/side-ear/annotate/images")
def api_side_ear_images(root: str = Query(...)):
    dataset = resolve_root(root)
    return {"ok": True, "images": list_side_ear_images(dataset)}


@app.get("/api/side-ear/annotate/image/{split}/{filename}")
def api_side_ear_image(split: str, filename: str, root: str = Query(...)):
    dataset = resolve_root(root)
    return FileResponse(safe_image_path(dataset, split, filename))


@app.get("/api/side-ear/annotate/label/{split}/{stem}")
def api_side_ear_label(split: str, stem: str, root: str = Query(...)):
    dataset = resolve_root(root)
    pts = read_side_ear_labels(dataset, split, stem)
    return {"ok": True, "points": pts}


@app.post("/api/side-ear/annotate/save")
def api_side_ear_save(body: SaveSideEarAnnotation):
    dataset = resolve_root(body.root)
    if body.image_width < 1 or body.image_height < 1:
        raise HTTPException(400, "invalid image dimensions")
    if len(body.points) != 4:
        raise HTTPException(400, "mark all 4 points (earlobe + 3 earring positions)")
    img_path = safe_image_path(dataset, body.split, body.filename)
    pixel_pts = [{"x": p.x, "y": p.y} for p in body.points]
    write_side_ear_labels(
        dataset,
        body.split,
        img_path.stem,
...
@app.post("/api/side-ear/annotate/split-val")
def api_side_ear_split_val(body: DatasetRoot, ratio: float = Query(0.15)):
    dataset = resolve_root(body.root)
    result = side_ear_split_val(dataset, val_ratio=ratio)
    save_side_ear_data_yaml_for_root(dataset)
    return {"ok": True, **result, "images": list_side_ear_images(dataset)}


@app.post("/api/side-ear/annotate/prepare-train")
def api_side_ear_prepare_train(body: DatasetRoot):
    dataset = resolve_root(body.root)
    side_ear_ensure_layout(dataset)
    if len(list((dataset / "images" / "val").glob("*"))) == 0:
        side_ear_split_val(dataset)
    save_side_ear_data_yaml_for_root(dataset)
    code, text = run_cmd("validate_dataset.py", "--data", str(DATA_YAML_SIDE))
    return {"ok": code == 0, "output": text}


def _train_worker_side(params: TrainParams) -> None:
    cmd = [
        PYTHON,
        str(ROOT / "python" / "train.py"),
        "--data",
...
def api_side_ear_train_status():
    return train_state_side.snapshot()


@app.post("/api/side-ear/model/export-library")
def api_side_ear_export_library():
    try:
        info = export_side_ear_web_library(
            ROOT, model_path=ONNX_PATH_SIDE if ONNX_PATH_SIDE.is_file() else None
        )
        if not info["has_model"]:
            raise HTTPException(400, "side-ear.onnx not found — train side-ear model first")
        export_dir = Path(info["export_dir"])
        ...
        return {
            ...
            "export_dir": str(export_dir),
            "zip": archive,
            "message": "Copy export/side-ear-library/ into your UI app.",
        }
    except FileNotFoundError as e:
        raise HTTPException(400, str(e)) from e


@app.get("/api/side-ear/model/download-library")
def api_side_ear_download_library():
    zip_path = ROOT / "export" / "side-ear-web-library.zip"
    if not zip_path.is_file():
        raise HTTPException(404, "Run Export side-ear library first")
    return FileResponse(zip_path, filename="side-ear-web-library.zip", media_type="application/zip")


@app.get("/side-ear/")
def side_ear_index():
    index_file = SIDE_EAR_WEB_DIR / "index.html"
    if not index_file.is_file():
        raise HTTPException(404, "training-web/side-ear/index.html not found")
    return FileResponse(index_file)


@app.get("/side-ear/assets/app.js")
def side_ear_asset_js():
    return FileResponse(SIDE_EAR_WEB_DIR / "app.js", media_type="application/javascript")
...
def main() -> None:
    ...
    print("Front-face earlobe training: http://127.0.0.1:8000/")
    print("Side-ear (4 points) training: http://127.0.0.1:8000/side-ear/")
